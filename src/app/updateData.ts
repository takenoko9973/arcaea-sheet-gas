...
function updateSongData(collectedSong: CollectionSong) {
  // 登録済みかどうか
  const registedSong = fetchRegistedSong(
  ...
  // 整合性確認
  const equalSongData = checkSongDataConsistency(
    registedSong,
    collectedSong.toSongData()
  );
  ...
  console.log(
    'update data of %s(%s)',
    registedSong.nameJp,
    registedSong.difficulty
  );
  const newSongData = updateSongDataWithDelta(
    registedSong,
    collectedSong.toSongData()
  );
...
function overwriteSongData(songData: Song) {
  ...
  SONG_SHEET.getRange(
    'A' + (registedRow + 1) + ':L' + (registedRow + 1)
  ).setValues(updateInfo);
}
